...
def _conservative_max_call_cost(model_id: str, case: dict[str, Any], max_output_tokens: int) -> float:
    ...
    # Conservative token upper bound: UTF-8 bytes plus 20k bytes of request/schema overhead.
    input_bytes = len(
...
def run_benchmark(*, root: Path, output_dir: Path, budget_usd: float, max_calls: int) -> dict[str, Any]:
    ...
    # Deliberate boring-first order: compare both models on CASE-001 before later cases.
